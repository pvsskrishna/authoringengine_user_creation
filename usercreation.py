...
path = r"C:\Users\saikr\Downloads\user_creation_data (1).xlsx"
...
for r in range(2, rows+1):
    ...
    #act_succ_message=" Successfully Added "
    #act_fail_text=" Password did not conform with password policy: Password must have numeric characters"
    succ_message=driver.find_element(By.XPATH, "//div[@class='alert alert-success']")
    succ_message_cont=driver.find_element(By.XPATH, "//div[contains(text(),'uccessful')]").text
    #fail_text = driver.find_element(By.XPATH, "//div[text()=' Password did not conform with password policy: Password must have numeric characters']").text
    if succ_message.is_displayed:
        if succ_message.text==succ_message_cont:
            XLutils.writeData(path, "Sheet1", r, 6, "created")
            time.sleep(2)

    else:
        #fail_text==act_fail_text:
        XLutils.writeData(path, "Sheet1", r, 6, "Failed")
        time.sleep(2)

    driver.find_element(By.XPATH, "//a[text()=' Back']").click()
    time.sleep(2)
